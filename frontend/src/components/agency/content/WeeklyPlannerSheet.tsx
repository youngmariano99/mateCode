  ...
  return (
    <div className="space-y-6 flex-1 flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Lado Izquierdo: Pasos 1, 2, 3 (8 columnas) */}
        ...
        {/* Lado Derecho: Checklists 4, 5 y Bloque 6 (4 columnas) */}
        <div className="lg:col-span-4 space-y-6">
          
          {/* BLOQUE 4: Batching Day */}
          <div className="bg-zinc-900/40 border border-zinc-800/60 p-5 rounded-3xl backdrop-blur-md space-y-4">
            <h3 className="text-xs font-black uppercase text-zinc-400 tracking-wider flex items-center gap-2 pb-2 border-b border-zinc-800/60">
              <span className="w-5 h-5 rounded-full bg-indigo-500/10 text-indigo-400 flex items-center justify-center text-[10px] font-black">4</span>
              <span>Día de Armado (Batching Day)</span>
            </h3>

            <div className="space-y-2">
              {[
                { key: 'setupGrabacion', label: '🎬 Set-up armado (trípode, luces, cámara limpia)' },
                { key: 'grabadoPost1', label: '📹 Grabé el Post 1' },
                { key: 'reseteoVisual', label: '👕 Cambié de remera o ángulo para el Post 2' },
                { key: 'subtitulosCentro', label: '✍️ Subtítulos grandes en el centro' },
                { key: 'cortesRapidos', label: '✂️ Cortes rápidos cada 3-5 segundos' },
                { key: 'archivoExportado', label: '💾 Archivos finales exportados' }
              ].map(item => {
                const isChecked = !!checklistBatching[item.key];
                return (
                  <button
                    key={item.key}
                    type="button"
                    onClick={() => {
                      setChecklistBatching({
                        ...checklistBatching,
                        [item.key]: !isChecked
                      });
                    }}
                    className={`w-full flex items-start gap-2.5 p-2 rounded-xl text-left text-xs transition-colors hover:bg-zinc-950/40 ${
                      isChecked ? 'text-zinc-200' : 'text-zinc-500'
                    }`}
                  >
                    {isChecked ? (
                      <CheckCircle2 size={14} className="text-indigo-400 shrink-0 mt-0.5" />
                    ) : (
                      <Circle size={14} className="text-zinc-750 shrink-0 mt-0.5" />
                    )}
          ...
          {/* BLOQUE 5: Subida y SEO */}
          <div className="bg-zinc-900/40 border border-zinc-800/60 p-5 rounded-3xl backdrop-blur-md space-y-4">
            <h3 className="text-xs font-black uppercase text-zinc-400 tracking-wider flex items-center gap-2 pb-2 border-b border-zinc-800/60">
              <span className="w-5 h-5 rounded-full bg-pink-500/10 text-pink-400 flex items-center justify-center text-[10px] font-black">5</span>
              <span>Subida & SEO (Algoritmo)</span>
            </h3>

            <div className="space-y-2">
              {[
                { key: 'nombreArchivoRenombrado', label: '📂 Nombre del archivo relevante (ej: video.mp4 ➡️ excel-tickets.mp4)' },
                { key: 'keywordsTexto', label: '✍️ Palabras clave de forma natural en el texto' },
                { key: 'hashtagsEspecificos', label: '🏷️ 3 a 5 hashtags muy específicos (B2B)' },
                { key: 'sinMarcasAgua', label: '🚫 Video limpio sin marcas de agua de otras redes' }
              ].map(item => {
                const isChecked = !!checklistSeo[item.key];
                return (
                  <button
                    key={item.key}
                    type="button"
          ...
          {/* BLOQUE 6: Análisis de Resultados */}
          <div className="bg-zinc-900/40 border border-zinc-800/60 p-5 rounded-3xl backdrop-blur-md space-y-4">
            <h3 className="text-xs font-black uppercase text-zinc-400 tracking-wider flex items-center gap-2 pb-2 border-b border-zinc-800/60">
              <span className="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center text-[10px] font-black">6</span>
              <span>Análisis de la Semana (Cierre)</span>
            </h3>

            <div className="space-y-3">
              <div>
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Total de DMs / Leads Automatizados</label>
                <input
                  type="number"
                  min={0}
                  value={totalDms}
                  onChange={e => setTotalDms(parseInt(e.target.value) || 0)}
              ...
              <div>
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Post con mejor Retención / Vistas</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Post más Guardado / Compartido</label>
                <input
                  type="text"
                  placeholder="Título del post..."
                  value={masCompartido}
                  onChange={e => setMasCompartido(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-850 p-2.5 rounded-xl text-xs text-white outline-none focus:border-emerald-500/50"
                />
              </div>

              <div>
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">¿Qué funcionó bien? (Para repetir)</label>
                <textarea
                  rows={2}
                  placeholder="Ej: El gancho sobre el dolor de Excel sirvió mucho"
                  value={queFunciono}
                  onChange={e => setQueFunciono(e.target.value)}
              ...
              <div>
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">¿Qué falló / se puede mejorar?</label>
                <textarea
                  rows={2}
                  placeholder="Ej: El video 3 quedó muy largo"
                  value={queFallo}
              ...
              <div>
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Decisión para la semana que viene</label>
                <textarea
                  rows={2}
                  placeholder="Ej: Hacer videos más cortos y probar otro plano visual"
                  value={decisionProximaSemana}
                  onChange={e => setDecisionProximaSemana(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-850 p-2.5 rounded-xl text-xs text-white outline-none resize-none focus:border-emerald-500/50"
                />
              </div>
            </div>
