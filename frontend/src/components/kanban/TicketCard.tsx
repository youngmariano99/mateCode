...
import { Sparkles, MessageSquare, Clock, AlertTriangle } from 'lucide-react';
...
export const TicketCard = ({ ticket, index }: { ticket: Ticket, index: number }) => {
    ...
    return (
        <>
            <div 
                ref={ref}
                className={`group relative p-4 bg-zinc-900 border rounded-xl transition-all duration-200 cursor-grab active:cursor-grabbing ${
                    isDragging ? 'opacity-20 scale-95 ring-2 ring-emerald-500/50' : 'opacity-100 hover:border-zinc-600 shadow-md'
                ...
                <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                        <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                            ticket.tipo === 'Bug' ? 'bg-red-500/10 text-red-500' : 
